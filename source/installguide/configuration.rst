...
#. In a new cluster, CloudStack adds the first primary storage server
   ...
   -  **Protocol.** For XenServer, choose either NFS, iSCSI, or
      PreSetup. For KVM, choose NFS, SharedMountPoint,CLVM, RBD or PowerFlex. For
...
#. In a new cluster, CloudStack adds the first primary storage server
   ...
   -  **Protocol.** For XenServer, choose either NFS, iSCSI, or
      PreSetup. For KVM, choose NFS, SharedMountPoint, CLVM, RBD or PowerFlex.
...
#. Provide the following information in the dialog. The information
   ...
   -  **Protocol.** For XenServer, choose either NFS, iSCSI, or
      PreSetup. For KVM, choose NFS, SharedMountPoint or PowerFlex. For vSphere
...
This plugin enables Dell EMC PowerFlex™ (v3.5 onwards) storage pools as a managed
primary storage in CloudStack for KVM hypervisor.

The ScaleIO storage installation/setup and creation of the ScaleIO storage pool is
out of scope of this document. For ScaleIO storage installation instructions, Refer
to ScaleIO product documentation:
https://www.dell.com/support/home/en-in/product-support/product/scaleio/docs.
To know more details about PowerFlex/ScaleIO, refer to
https://cpsdocs.dellemc.com/bundle/PF_KNOW/page/GUID-D6DFA46A-6085-47CE-88A9-B503EFC6CFD2.html.

When this storage pool is used with Compute or Disk Offerings, an administrator is able to
build an environment in which a root or data disk that a user creates
leads to the dynamic creation of a PowerFlex/ScaleIO volume, which has
guaranteed performance with the ScaleIO Data Client (SDC) limits for the
volume specified using the details parameters: bandwidthLimitInMbps &
iopsLimit (both defaulted to 0 - unlimited). Such a PowerFlex volume is
associated with one (and only ever one) CloudStack volume, so performance
of the CloudStack volume does not vary depending on how heavily other tenants
